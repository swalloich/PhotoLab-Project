...
public class Picture extends SimplePicture
{
	...
	public void copyNoBlack(Picture fromPic, int startRow, int startCol)
	{
		Pixel fromPixel = null;
		Pixel toPixel = null;
		Pixel[][] toPixels = this.getPixels2D();
				...
				& toRow < toPixels.length; fromRow++, toRow++)
		{
					...
					&& toCol < toPixels[0].length; fromCol++, toCol++)
			{
				fromPixel = fromPixels[fromRow][fromCol];
				toPixel = toPixels[toRow][toCol];
				if (fromPixel.getColor().getRGB() != Color.BLACK.getRGB())
				{
					toPixel.setColor(fromPixel.getColor());
				}
			}
	...
	private int randomPlace(Picture image, String widthOrHeight)
	{
		int coordinate = 0;
		double wh = Math.random();

		if (widthOrHeight.equals("width") || widthOrHeight.equals("w"))
		{
			coordinate = (int) (wh * image.getWidth());
		} else if (widthOrHeight.equals("height") || widthOrHeight.equals("h"))
		{
			coordinate = (int) (wh * image.getHeight());
		}

		return coordinate;
	}

	public void addHearts(Picture picture)
	{
